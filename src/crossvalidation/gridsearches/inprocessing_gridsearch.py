...
class InProcessingGridSearch(GridSearch):
    """
    ...
    def fit(self, x_train:list, y_train:list, demographics: list, fold:int):
        ...
        for _, combination in enumerate(self._combinations):
            ...
            for f, (train_index, validation_index) in enumerate(splitter.split(x_train, y_train, demographics)):
                ...
                history = model.fit(xx_train, yy_train, demographics_train, x_val, y_val, demographics_val)
                self.loss_histories['_'.join([str(c) for c in combination])][f].append(history)
                
                y_pred, y_val = model.predict(x_val, y_val, demographics_val)
                y_proba = model.predict_proba(x_val, demographics_val)
        ...
        model.fit(x_train, y_train, demographics, x_val, y_val, demographics_val)
        model.save(extension='best_model_f{}'.format(fold))
        self._best_model = model
        
            
    def predict(self, x_test: list, y_test, demographics_test) -> list:
        ...
        """
        return self._best_model.predict(x_test, y_test, demographics_test)
        
        
    def predict_proba(self, x_test:list, demographics_test) -> list:
        return self._best_model.predict_proba(x_test, demographics_test)

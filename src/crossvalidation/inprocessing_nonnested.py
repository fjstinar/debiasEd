...
class InProcessingNonNestedXVal(CrossValidator):
    ...
    def __init__(
        ...
        self._name = 'in processing nonnested cross validator'
        self._notation = 'innonnested_xval'
    ...
    def crossval(self, x:list, y:list, demographics:list) -> dict:
        ...
        for f, (train_index, test_index) in enumerate(self._outer_splitter.split(x, y, demographics)):
            ...
            results[f]['loss_history'] = model.fit(x_train, y_train, dem_train, x_train, y_train, dem_train)
            results[f]['best_estimator'] = model.save_fold(f)

            # Predict
            y_pred, y_test = model.predict(x_test, y_test, dem_test)
            y_proba = model.predict_proba(x_test, dem_test)

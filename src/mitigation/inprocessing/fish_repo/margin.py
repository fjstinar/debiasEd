...
from mitigation.inprocessing.fish_repo.errorfunctions import signedStatisticalParity, precomputedLabelError, precomputedLabelStatisticalParity
from mitigation.inprocessing.fish_repo.utils import sign
from mitigation.inprocessing.fish_repo.decision_stump import buildDecisionStump
from mitigation.inprocessing.fish_repo import boosting
import mitigation.inprocessing.fish_repo.lr as lr
import numpy
import random
import mitigation.inprocessing.fish_repo.svm as svm
...
class lrSKLMarginAnalyzer(marginAnalyzer):
   def __init__(self, data, protectedIndex, protectedValue):
      # self.splitData(data)
      self.margin = lr.lrDetailedSKL(data[0])

"""
This file is taken from Zafar et al. at
https://github.com/mbilalzafar/fair-classification

It serves as our baseline for comparison and is only used by the
`PpercentBaseline` module.
"""
...
def train_model(x, y, x_control, loss_function, apply_fairness_constraints,
...
def compute_p_rule(x_control, class_labels):
    ...
    ratio = frac_prot_pos / frac_non_prot_pos
    p_rule = min((ratio, 1./ratio)) * 100.0
    print()
...
def get_one_hot_encoding(in_arr):
    ...
    for k in in_arr:
        if not isinstance(k, (np.float, np.int, float, int)):
            print(str(type(k)))

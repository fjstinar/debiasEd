...
# from mitigation.inprocessing.sikdar_repo.fairnessMetric import disparateMistreatment
# import math
...
#         ts_pred = self.cls.predict(self.data[0])
#         reward = self.reward_function(self.data[1], self.data[2], ts_pred)
...
#             ts_pred = self.cls.predict(self.data[0])
#             score_ = (self.cls.score(self.data[0], self.data[2]))
#             # reward function based on the fairness measure..
#             reward = torch.tensor(self.reward_function(self.data[1], self.data[2], ts_pred), dtype=torch.float32)
...
#             if self.data[2][i] == 0:
#                 if self.data[1][i] == 0:
#                     ax.scatter(self.data[0][i][0], self.data[0][i][1], c='r', marker='x')
#                 else:
#                     ax.scatter(self.data[0][i][0], self.data[0][i][1], c='r', marker='1')
#             else:
#                 if self.data[1][i] == 0:
#                     ax.scatter(self.data[0][i][0], self.data[0][i][1], c='b', marker='x')
#                 else:
#                     ax.scatter(self.data[0][i][0], self.data[0][i][1], c='b', marker='1')
...
#     def clsScore(self):
#         return self.cls.score(self.data[0], self.data[2])
...
#         print('starting parameter', self.cls.coef_)
#         ts_pred = self.cls.predict(self.data[0])
#         reward = self.reward_function(self.data[1], ts_pred)
#         self.rewards.append((self.cls.coef_[0][0], self.cls.coef_[0][1], self.cls.intercept_[0], reward))
#         for _ in tqdm(range(epochs)):
#             param = np.concatenate((self.cls.coef_, self.cls.intercept_.reshape(1, -1)), axis=1)
#             inp = torch.tensor(param, dtype=torch.float32)
...
#             ts_pred = self.cls.predict(self.data[0])
#             score_ = self.cls.score(self.data[0], self.data[2])
#             reward = torch.tensor(self.reward_function(self.data[1], ts_pred), dtype=torch.float32)
#             self.rewards.append((self.cls.coef_[0][0], self.cls.coef_[0][1], self.cls.intercept_[0], reward.item(),
#                                  score_))
#             # discount = self.avgReward()
#             # loss = torch.sum(torch.log(out) * (reward - discount))
...
#         ts_pred = self.cls.predict(self.data[0])
#         reward = self.reward_function(self.data[1], ts_pred, self.data[2])
...
#                 score_ = self.cls.score(self.data[0], self.data[2])
#                 # Calculate reward
#                 r_f = self.reward_function(self.data[1], ts_pred, self.data[2])
#                 if r_f is None:
#                     reward = 0
#                 else:
#                     #print(r_f, score_)
...
#                 ts_pred = self.cls.predict(self.data[0])
#                 score_ = self.cls.score(self.data[0], self.data[2])
#                 reward_ = self.reward_function(self.data[1], ts_pred, self.data[2])
...
#         else:
#             pass
#         ts_pred = self.cls.predict(data[0])
#         score_ = self.cls.score(data[0], data[2])
#         # reward = self.reward_function(data[1], ts_pred, data[2])
...
#         # print(f'disparate mistreatment: {disparateMistreatment(data[1], data[2], ts_pred)}')
#         return ts_pred, score_
...
#         r_1 = metric_1(self.data[1], ts_pred, self.data[2])
#         r_2 = metric_2(self.data[1], ts_pred, self.data[2])
#         return r_1, r_2

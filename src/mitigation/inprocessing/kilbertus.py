...
from mitigation.inprocessing.kilbertus_repo.fair_logistic_regression import FairLogisticRegression
...
class KilbertusInProcessor(InProcessor):
    ...
    def __init__(self, settings: dict):
        ...
        self._name = 'kilbertus et al.'
        self._notation = 'kilbertus'
        self._inprocessor_settings = self._settings['inprocessors']['kilbertus']
        self._information = {}
        self._fold = -1
    ...
    def _init_model(self):
        ...
        self.model = FairLogisticRegression(
            opt=self._inprocessor_settings['optimiser'],
            random_state=self._settings['seeds']['inprocessor']
        )
    ...
    def fit(self, 
        ...
        n_epochs = int(
            self._inprocessor_settings['epochs'] * self._inprocessor_settings['batchsize'] / len(x_train)
        )
        n = len(x_train)
        d = len(x_train[0])
        p = 1

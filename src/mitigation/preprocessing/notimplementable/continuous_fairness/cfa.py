...
class ContinuousFairnessAlgorithm():
    ...
    def _getTotalBarycenter(self, group_histograms_raw):
        ...
        total_bary = ot.bregman.barycenter(np.array(group_histograms_raw),
                                           self.__lossMatrix,
                                           self.__regForOT,
    ...
    def _get_group_barycenters(self, total_bary, group_histograms):
        ...
        for groupName in group_histograms:
            ...
            group_barycenters[groupName] = ot.bregman.barycenter(np.array(groupMatrix),
                                                                 self.__lossMatrix,
                                                                 self.__regForOT,
                                                                 weights=weights,
                                                                 verbose=True,
    ...
    def _calculateFairReplacementStrategy(self, group_barycenters, group_histograms_raw):
        ...
        for groupName in self.__groupColumnNames:
            ...
            ot_matrix = ot.emd(np.array(group_histograms_raw[groupName]),
                               np.array(group_barycenters[groupName]),
                               self.__lossMatrix)
...
#             plt.show()
    ...
    def _replaceRawByFairScores(self, groupFairScores):
        ...
        def replace(rawData, oldValue, newValue):
            ...
            fairScores = pd.DataFrame(groupFairScores[groupName])
            print('FAIRSCORES')
            print(fairScores)
            print(type(fairScores))
            print(self.__bin_edges)
            for index, column in enumerate(fairScores.columns):
                fairScore = list(fairScores[column])
                ...
                print('replacements')
                print(len(replaceAtIndex), len(newValue), len(fairScore))
                print(fairScore)
    ...
    def run(self):
        ...
        group_histograms_raw = group_histograms_raw # needs to be numpy array

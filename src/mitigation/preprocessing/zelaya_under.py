...
class ZelayaUnderPreProcessor(PreProcessor):
    ...
    def __init__(self, settings: dict):
        ...
        self._name = 'zelayaunder et al.'
        self._notation = 'zelayaunder'
    ...
    def fairCorrectUnder(self, df, pa, fav, d=1):
        ...
        if fav_pos_size > len(fav_pos):
            corr_fav_pos = fav_pos.sample(fav_pos_size, replace=True)
        else:
            corr_fav_pos = fav_pos.sample(fav_pos_size, replace=False)

        if unfav_neg_size > len(unfav_neg):
            corr_unfav_neg = unfav_neg.sample(unfav_neg_size, replace=True)
        else:
            corr_unfav_neg = unfav_neg.sample(unfav_neg_size, replace=False)
    ...
    def fit_transform(self, 
        ...
        corrected_df = self.fairCorrectUnder(full_data, 'demographic', 1, d=self._preprocessor_settings['d'])

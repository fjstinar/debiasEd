...
class StandardScalingSVCClassifier(Predictor):
    ...
    def __init__(self, settings:dict):
        ...
        self._name = 'standard-scaled-svc'
        self._notation = 'sssvc'
        self._model_settings = settings['predictors']['sssvc']
        self._fold = -1

    def update_settings(self, settings):
        self._model_settings.update(settings)
        self._settings['predictors']['sssvc'].update(settings)
        self._choose_fit()
    ...
    def _init_model(self):
        ...
        self.model = make_pipeline(
            StandardScaler(),
            SVC(kernel=self._model_settings['kernel'], C=self._model_settings['C'], probability=True)
        )
    ...
    def save(self, extension='') -> str:
        ...
        with open('{}svc_{}.pkl'.format(path, extension), 'wb') as fp:
            pickle.dump(self, fp)
        return '{}svc_{}'.format(path, extension)
